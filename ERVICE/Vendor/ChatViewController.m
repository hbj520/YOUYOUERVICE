...
- (void)viewDidLoad {
    ...
    NSString *nickName = [SCUserProfileEntity getNickNameWithUsername:self.conversation.conversationId];
    if (nickName) {
        self.navigationItem.title = nickName;
    }else{
        [[MyAPI sharedAPI] userInfoWithUserId:self.conversation.conversationId result:^(BOOL success, NSString *msg, id object) {
            if (success) {
                HuanXinUserInfo *userinfo = object;
                self.navigationItem.title = userinfo.nickname;
                [SCUserProfileEntity saveUserProfileWithUsername:self.conversation.conversationId
                           ...
                           modelForMessage:(EMMessage *)message
{
  ...
  __block  NSString *nickName = [SCUserProfileEntity getNickNameWithUsername:message.from];
    if (nickName) {
        ...
        [[MyAPI sharedAPI] userInfoWithUserId:message.from result:^(BOOL success, NSString *msg, id object) {
            if (success) {
                HuanXinUserInfo *userinfo = object;
                [SCUserProfileEntity saveUserProfileWithUsername:message.from
                                                     forNickName:userinfo.nickname
                                                   avatarURLPath:userinfo.icon];
                //保存到本地后，从本地获取
